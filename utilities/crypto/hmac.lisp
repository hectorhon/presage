...
(defmacro define-compute-hmac-function (function-name hash-function-name hash-output-length block-size)
  `(defun ,function-name (secret-key text output-array)
     ...
     (declare (type (simple-array (unsigned-byte 8) (,hash-output-length)) output-array))
     (let ((arr-0 output-array)         ; using it for temporary values too
           (arr-1 (make-array ,block-size :element-type '(unsigned-byte 8)))
           (arr-2 (make-array (+ ,block-size (length text)) :element-type '(unsigned-byte 8)))
           (arr-3 (make-array (+ ,block-size ,hash-output-length) :element-type '(unsigned-byte 8))))
       (declare (dynamic-extent arr-1 arr-3))
       (if (> (length secret-key) ,block-size)
           (progn (,hash-function-name secret-key arr-0)
                  (map-into arr-1 #'identity arr-0))
           (let ((i 0))
       ...
       (,hash-function-name arr-2 arr-0)
       (loop :for byte :across arr-0    ; Fill up second half of arr-3
          :for i :from ,block-size
          :do (setf (aref arr-3 i) byte))
       (,hash-function-name arr-3 arr-0))))
